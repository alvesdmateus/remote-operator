...
class AnsibleManager:
    def __init__(
        self, ssh_connection, playbook_dir="/etc/ansible/playbooks", debug=False
    ):
        self.debug = debug
...
ssh_args = -o StrictHostKeyChecking=no
        ...
        if stderr:
            print(
                f"Error configuring host key checking: '{command}' : {stderr}"
            )
        else:
            print(f"Success Configuring host key checking")

    def check_ansible_installation(self):
        stdout, stderr = self.ssh_connection.execute_command(
            "ansible --version"
        )
    ...
    def setup_ansible(self):
        ...
        if self.check_ansible_installation():
        ...
        if self.check_ansible_installation():
            print("Ansible setup completed successfully")
    ...
    def register_private_key(self, private_key_file):
        ...
        commands = [
            f"echo '{private_key}' | sudo tee -a {self.private_key_file}",
    ...
    def get_playbook_output(self, playbook_name):
        ...
        if stderr:
            print(
                f"Error retrieving output for playbook '{playbook_name}': {stderr}"
            )
        else:
            return stdout
    ...
    def register_hosts(self, hosts_input, group):|
        ...
        stdout, stderr = self.ssh_connection.execute_command(
            f"echo '{inventory_content}' | sudo tee -a {self.inventory_file}"
        )
        if stderr:
